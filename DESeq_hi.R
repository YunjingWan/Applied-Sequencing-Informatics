...
CountTable <- read.table("/Users/staceyvann/Desktop/2022Spring/AppliedSequencing/Final/PRAMEhi.csv", header=TRUE,sep = ",") 
...
samples <- data.frame(sample = c("hi_ctrl1","hi_ctrl2","hi_ctrl3","hi_siRNA1","hi_siRNA2","hi_siRNA3"),
                      batch = c("1","2","3","1","2","3"),
                      condition = c("Ctrl","Ctrl","Ctrl","siRNA","siRNA","siRNA"))
samples$batch <- factor(samples$batch,levels=c("1","2","3"))
samples$condition <- factor(samples$condition,levels=c("Ctrl","siRNA"))
...
#Comparison
comparisons = rbind(c("Ctrl","siRNA"))
...
#Ctrl_siRNA_df<-as.data.frame(counts(dds))

baseMeanCtrl = rowMeans(counts(dds,normalized=TRUE)[,dds$condition == "Ctrl"])
baseMeansiRNA = rowMeans(counts(dds,normalized=TRUE)[,dds$condition == "siRNA"])

res1 = cbind(as.data.frame(res), baseMeanCtrl, baseMeansiRNA)
...
write.csv(res1, "/Users/staceyvann/Desktop/2022Spring/AppliedSequencing/Final/DEG_hi.csv", row.names=TRUE)
...
pRNA <- ggplot(sigRNA, aes(log2FoldChange, log10.pvalue))+
  ...
  ggtitle("Differentially expressed genes in PRAMEhi silencing experiment")+
  theme_bw() + 
  theme(panel.background = element_blank())+
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1))+
...
write.csv(DEG_sig, "/Users/staceyvann/Desktop/2022Spring/AppliedSequencing/Final/DEGdeseq2_hi.csv", row.names=TRUE)
...
dotplot(go, title = "GO analysis of upregulated genes in PRAME silencing")

dotplot(kegg, title = "KEGG analysis of upregulated genes PRAME silencing")
...
dotplot(go, title = "GO analysis of down-regulated genes PRAME silencing")

dotplot(kegg, title = "KEGG analysis of down-regulated genes PRAME silencing")
